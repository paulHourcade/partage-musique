...
export default function App1() {
  ...
  const [themeSearchError, setThemeSearchError] = useState("");
    ...
    const initPlayer = async () => {
      try {
        ...
        createdPlayer = new window.Spotify.Player({
          name: "Partage Musique · App 1 Player",
  ...
  const searchSpotifyByTheme = async (queryText) => {
    if (!queryText || !queryText.trim()) {
      setThemeResults([]);
      setThemeSearchError("");
      return;
    ...
    try {
      ...
      const enrichedQuery = `${queryText} musique`;
      const res = await fetch(
        `/api/spotify-search?q=${encodeURIComponent(enrichedQuery)}`
      );
      const text = await res.text();
      ...
      const data = JSON.parse(text);
      setThemeResults(Array.isArray(data) ? data : []);
    } catch (err) {
      console.error("Theme Spotify error:", err);
      setThemeResults([]);
      setThemeSearchError("Impossible de charger les résultats par thème");
    } finally {
    ...
    <div style={styles.page}>
      ...
      <div style={styles.card}>
        ...
        >
          <div style={styles.sectionHeader}>
            <div>
              <div style={styles.sectionEyebrow}>Recherche par thème</div>
              <h2 style={styles.sectionTitle}>✨ Ambiance / thème</h2>
            </div>
          </div>
          ...
          <div style={styles.inputCol}>
            <input
              style={styles.input}
              value={themeQuery}
              placeholder="Exemple : soirée, chill, sport, années 80, romantique..."
              onChange={(e) => {
                const value = e.target.value;
                setThemeQuery(value);
                setAddError("");
                ...
                clearTimeout(spotifyTimeoutRef.current);
                spotifyTimeoutRef.current = setTimeout(() => {
                  searchSpotifyByTheme(value);
                }, 400);
              }}
            />

            {isThemeSearching && (
              <div style={styles.infoText}>Recherche par thème en cours...</div>
            )}
            {themeSearchError && <div style={styles.errorText}>{themeSearchError}</div>}
              ...
              <div style={styles.results}>
                {themeResults.map((t) => (
                  ...
                  >
                    ...
                    <button
                      style={styles.themePickButton}
                      onClick={(e) => {
                        e.stopPropagation();
                        selectSpotifyTrack(t);
                      }}
                    >
                      Choisir
                    </button>
            ...
            {!isThemeSearching && themeQuery.trim() && themeResults.length === 0 && !themeSearchError && (
              <div style={styles.infoText}>
                Aucun résultat pour ce thème pour le moment.
              </div>
            )}
          </div>
        </div>
